...
export default function ScalableSlide() {
  ...
  useEffect(() => {
    ...
    const gc = gsap.context(() => {
      ...
      refTime.current.fromTo(refPo.current, { y: '0%' }, {
        width: '33.33%', duration: 3, onStart: () => {
          if (refSlide.current.index === 0) return;

          refItems.current && animation(refItems.current[2], refItems.current[0]);
      ...
      refTime.current.to(refPo.current, {
        width: '66.66%', duration: 3, onStart: () => {
  ...
  useEffect(() => {
    if (inView) refTime.current?.resume(); else refTime.current?.paused();
  }, [inView]);
