...
import React, { Fragment, ReactNode, memo, useEffect, useState, useRef } from 'react';
...
const ListTable: React.FC<ListTableProps> = ({
  ...
  const handleScroll = () => {
    if (feedContainerRef && feedContainerRef?.current) {
      ...
      if (container) {
        ...
        if (isScrolledToBottom && !hasIncrementedPageRef.current) {
          onFetch();
        }
      }
    }
  ...
  return (
    <TableContainer className={styles.container} ref={feedContainerRef}>
      <Table variant="simple" className={className}>
        {!noHeader &&
          (HeaderComponent ? (
            HeaderComponent
          ) : (
            <Thead position={"sticky !important" as any} top={"0px !important" as any}>
              <Tr>
                {columns.map((v) => {
                  return (
