...
import { formDappDropdownSignal, formDappInputSignal, formDappToggleSignal } from '../signals/useFormDappsSignal';
  ...
  const onSubmitFormTokenGeneration = async () => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        for (const block of blocks) {
          ...
          if (!blockTemp?.name || isEmpty(blockTemp?.name)) {
            errors.push({ key: 'tokenomic_name', error: `Allocation #${index} name is required!` });
          }
          if (!blockTemp?.total_amount || isEmpty(blockTemp?.total_amount)) {
            errors.push({ key: 'tokenomic_amount', error: `Allocation #${index} amount is required!` });
          }
          if (!blockTemp?.address || isEmpty(blockTemp?.address)) {
            errors.push({ key: 'tokenomic_address', error: `Allocation #${index} address is required!` });
          }

          if(blockTemp?.is_vesting) {
            if (!blockTemp?.cliff || isEmpty(blockTemp?.cliff)) {
              errors.push({ key: 'tokenomic_cliff_amount', error: `Allocation #${index} cliff amount is required!` });
            }
            if (!blockTemp?.duration || isEmpty(blockTemp?.duration)) {
              errors.push({ key: 'tokenomic_duration_amount', error: `Allocation #${index} duration amount is required!` });
            }
          }
        }
      }
      ...
      const getTokenomicsDefault: ITokenomics[] = () => {
        if (getTotalSupply(blockMapping?.allocation as unknown as ITokenomics[]) === 0) {
          return [
            {
              name: "Foundation",
