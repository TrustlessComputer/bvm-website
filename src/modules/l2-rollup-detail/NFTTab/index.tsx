...
const NFTTab = (props: IProps) => {
  ...
  const [selectedRollup, setSelectedRollup] =
    useState<IRollupDetail | undefined>(RollupAll);
  ...
  const list = useMemo(() => {
    ...
    rollupCollections.forEach((data) => {
      if (
        data.balances &&
        (selectedRollup?.rollup?.id === 0 ||
          (selectedRollup && data?.rollup?.id === selectedRollup?.rollup?.id))
      )
        transactions = [
          ...transactions,
  ...
  const listRollup = useMemo(() => {
    ...
    if (rollupDetails && rollupCollections) {
      rollupDetails.forEach((rollupDetail) => {
        const index = rollupCollections.findIndex(
          (collection) =>
            collection.balances &&
            collection.rollup.id === rollupDetail.rollup?.id,
        );
        if (index >= 0) {
  ...
  const [selectedCollection, setSelectedCollection] =
    useState<INFT | undefined>(undefined);
    ...
    return (
      ...
      >
        {listRollup.map((rollupDetail) => {
          ...
          const numCollection = rollupCollections.reduce(
            (result, collection) => {
          ...
          return (
            <Flex
              bg={
                rollupDetail?.rollup?.id === selectedRollup?.rollup.id
                  ? '#fa4e0e'
                  : '#F5F5F5'
              }
              direction={'row'}
