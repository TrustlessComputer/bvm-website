...
import { estimateBridge } from '@/modules/Bridges/utils';
import React from 'react';

const InformationBox = () => {
  const { values } = useFormikContext();
  const { toToken, fromToken, fromNetwork, toNetwork, isQRCode } =
      ...
      {!isQRCode && (
        <>
          <Text fontSize={'14px'} fontWeight={'500'} color={'#000'}>
            Fee ~{minAmount} {fromToken?.symbol}
          </Text>
          ...
          <Text fontSize={'12px'} fontWeight={'400'} color={'#657786'}>
            ...
            {fromToken?.network?.nativeCurrency?.symbol} for gas fees to
