...
\`\`\`js
function solve(uint256[] memory a, uint256[] memory u, uint256[] memory v) external returns (uint256);
\`\`\`
- $a$: Array of length $n$. For $0$ &le; $i$ < $n$, $a_i$ is the cost of bulding a library in city $i$.
- $u$, $v$: Two arrays of length $m$. For $0$ &le; $i$ < $m$, $u_i$ and $v_i$ describe the $i$-th road. It is guaranteed that no two roads connect the same pair of cities, and no road connects a city to itself.
