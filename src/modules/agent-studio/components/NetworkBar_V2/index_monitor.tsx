...
type Props = {
  mainData: {
    MainnetList: OrderItem[];
    TestnetList: OrderItem[];
  };
};

const NetworkBar = (props: Props) => {
  const { mainData } = props;
  const dispatch = useAppDispatch();
  const { viewMode, isFetchedAllOrdersV2 } = useAppSelector(
    getL2ServicesStateSelector,
  );

  const isMainnet = viewMode === 'Mainnet';

  if (!isFetchedAllOrdersV2)
    return (
      <Flex
        flexDir={'row'}
        align={'center'}
