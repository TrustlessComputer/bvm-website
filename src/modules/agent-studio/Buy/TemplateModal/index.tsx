...
import Item from '@/modules/agent-studio/Buy/TemplateModal/Item';
import Link from 'next/link';
import { FAKE_DATA_PACKAGE } from '@/modules/agent-studio/Buy/TemplateModal/data';
      ...
      <div className={s.wrapperItem}>
        {FAKE_DATA_PACKAGE.map((item) => {
          return (
            <div onClick={onClose}>
              <Item
                heading={item.template}
                price={item.price}
                content={item.content}
                iconCheck={item.check}
                color={item.color}
                link={item.link}
                option={item.options}
              />
