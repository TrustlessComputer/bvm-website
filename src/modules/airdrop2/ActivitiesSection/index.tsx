...
const DELAY_ACTIVITY_ITEM = 0.1;

const MODULAR_TW_LINK =
  ...
  return `
    <li>
      <div style='display: flex; flex-direction: row; align-items: end'>
        <span style='font-size: 14px; line-height: 160%; color: black; font-weight: 700'>${
          p.key
        }</span>
      ...
      <IncreaseNumber
        from={new BigNumber(
          new BigNumber(p.value)
            .minus(new BigNumber(p.value).div(p.diffNumb || 300))
            .toFixed(0, BigNumber.ROUND_CEIL),
        ).toNumber()}
        to={new BigNumber(
          new BigNumber(p.value).toFixed(0, BigNumber.ROUND_CEIL),
        ).toNumber()}
    ...
    return [
      {
        ...
        desc: `
          <ul>
            ${NormalRow({
              key: 'Activities:',
              value: 'Experience fully onchain AI on Bitcoin and win prizes.',
            })}
            ${LinkRow({
              key: 'Bitcoin L2:',
              value: 'Eternal AI',
      ...
      },
      {
        ...
        desc: `
          <ul>
            ${NormalRow({
              key: 'Activities:',
              value: 'Play 8 different fully on-chain games to earn rewards.',
            })}
            ${LinkRow({
              key: 'Bitcoin L2:',
      ...
      },
      {
        ...
        desc: `
          <ul>
            ${NormalRow({
              key: 'Activities:',
              value: 'Trade BRC-20 perpetual futures on-chain.',
            })}
            ${LinkRow({
              key: 'Bitcoin L2:',
              value: 'Naka',
      ...
      //           key: 'Activities:',
      //           value:
      //             'Learn about modular blockchain architecture via a fun Lego game.',
      //         })}
      //         ${LinkRow({
      //           key: 'Bitcoin L2:',
      //           value: 'Modular',
      ...
      //   },
      {
        ...
        desc: `
          <ul>
            ${NormalRow({
              key: 'Activities:',
              value: 'Participate in a charity run.',
            })}
            ${LinkRow({
              key: 'Bitcoin L2:',
              value: 'Alpha',
      ...
      switch (type) {
          ...
          if (gameReport && gameReport.total_game && gameReport.total_txs) {
            ...
            component2 = (
              <Flex
                ...
                onClick={() => {
                  window.open(
                    'https://explorer.testnet.bitcoinarcade.xyz/',
                    '_blank',
                  );
                }}
              >
          ...
          if (nakaVolume && nakaVolume.usd_volume) {
            component1 = (
              <Flex
                ...
                onClick={() => {
                  window.open(
                    'https://explorer.nakachain.xyz/address/0x43eF235efF5d8Aa29D34bCB7AE42dCFA6A86477e',
                    '_blank',
                  );
                }}
              >
        ...
        case ActivityType.Social: {
          ...
          if (alphaRun && alphaRun.total_distance && alphaRun.total_reward) {
            ...
            component2 = ReportRow({
              key: 'charity raised',
              value: new BigNumber(alphaRun.total_reward.toString())
                .div(1e8)
                .times(btcPrice)
                .toString(),
              maxDigit: 2,
  ...
  const renderItem = (item: GameItemProps) => {
    return (
      <Fade
        from={{ y: 10 }}
        to={{ y: 0 }}
        delay={DELAY_ACTIVITY_ITEM * TASKS.indexOf(item)}
      >
        <Flex
          flexDir="column"
          className={cx(
            styles.container_item,
            // [ActivityType.AI].includes(item?.type) ? styles.special : '',
          )}
          key={item.title}
        >
          <div className={styles.container_item_header}>
            <Text color="black" fontSize="16px" fontWeight="500">
              {item.title}
              {!!item.subTitle && (
                <span style={{ fontWeight: '400' }}>{item.subTitle}</span>
              )}
            </Text>
              ...
              <div>
                <Tooltip
                  ...
                  label={
                    <Flex direction="column" color="black" gap={'4px'}>
                      ...
                      <Text>
                        - Token Airdrop for the Top 3 most-used models - To Be
                        Announced.
                      </Text>
                      <Text>
                        - Token Airdrop for the Top 3 AI models with the most
                        likes on X.
                      </Text>
                      <Text>
                        - Token Airdrop for the Top 3 users who use the AI
                        models the most.
                      </Text>
                      <Text>
                        - Token Airdrop for the Top 3 users who use the AI
                      ...
                      <Text>
                        - Token Airdrop also extends to $BVM holders, $GM
                        holders, Perceptrons holders, Alpha OGs, NakaChain OGs,
                        Bitcoin Arcade OGs, and users of other Bitcoin L2
                        platforms powered by BVM.
                      </Text>
                    </Flex>
                  }
                ...
                >
                  ...
                  >
                    ...
                    >
                      <path
                        d="M6.66667 0.333984C2.98667 0.333984 0 3.32065 0 7.00065C0 10.6807 2.98667 13.6673 6.66667 13.6673C10.3467 13.6673 13.3333 10.6807 13.3333 7.00065C13.3333 3.32065 10.3467 0.333984 6.66667 0.333984ZM7.33333 10.334H6V6.33398H7.33333V10.334ZM7.33333 5.00065H6V3.66732H7.33333V5.00065Z"
                        fill="#FFFFFF"
                      />
                    </svg>
                  </Flex>
        ...
        >
          ...
          <Fade from={{ y: 10 }} to={{ y: 0 }} delay={0.2}>
            <Text
              ...
              textAlign={'center'}
            >
              Interact and engage with any dapps within the following Bitcoin
              L2s to earn a share of rewards from BVM AIRDROP SEASON 2
            </Text>
          </Fade>
        </Flex>
