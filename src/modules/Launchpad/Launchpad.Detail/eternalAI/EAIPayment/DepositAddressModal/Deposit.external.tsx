...
import DepositWarning from '@/modules/Launchpad/Launchpad.Detail/eternalAI/EAIPayment/DepositAddressModal/Deposit.warning';
import DepositSubmitHash from '@/modules/Launchpad/Launchpad.Detail/eternalAI/EAIPayment/DepositAddressModal/Deposit.submitHash';
...
interface IProps {
  isShowWarningHardcap: boolean;
}

const DepositExternal = ({ isShowWarningHardcap }: IProps) => {
  ...
  const onRequestSolWallet = async () => {
    try {
      if (compareString(selectToken?.coin, 'SOL')) {
        await paymentEAIApi.requestSolWallet();
      }
    } catch (error) {
      //
    }
