...
import useFormatHoldingFET from '@/modules/Launchpad/Launchpad.Detail/eternalAI/content/tasks/helpers/holdingAirdropMessage/useFormatHoldingFET';
...
const HoldFETAirdrop = (props: { index: number; isVerifyTW?: boolean }) => {
  const { currentLaunchpad } = useLaunchpadContext();
  const allowSAVM = useFormatHoldingFET();
  ...
  const savmOGMessage =
    Number(allowSAVM.amount.fet_amount) > 0 ? (
      <p>
        Congratulations! You've earned a reservation for{' '}
        {<span>{formatCurrency(allowSAVM.amount.fet_airdrop, 0)}</span>} $EAI
        tokens, which will be available after the upcoming TGE.
      </p>
    ) : (
      <>
        <p>The more $FET you hold, the larger your $EAI airdrop is.</p>
  ...
  const onClickShare = async () => {
    ...
    try {
      if (isNeedClaimBTCPoint) {
        shareTw({
          fee: allowSAVM.amount.fet_amount,
          point: allowSAVM.amount.fet_airdrop,
      ...
      <Flex className={s.shareTw}>
        <Flex justifyContent={'space-between'} gap={'12px'}>
          <Flex direction="column">
            <Text className={s.title}>Are you holding $FET?</Text>
