...
const shareTw = (params: {
  ...
  const content = `I've spent ${amount} BTC on gas fees and earned ${formatCurrency(
    params.point,
    0,
    0,
  )} points for @Naka_Chain launchpad at nakachain.xyz/launchpad.\n\nTheir mainnet launched in Jan 2024 and has already hit 2M transactions!\n\nIf you missed out on Merlin, then don’t fade NakaChain!`;
...
const BitcoinOG = (props: { index: number }) => {
  ...
  const btcOGMessage =
    Number(allowBTC.amount.fee) > 0 ? (
      <p>
        You're a true Bitcoin OG! You've spent{' '}
        {<span>{formatCurrency(allowBTC.amount.fee, 0, 6, 'BTC')}</span>} BTC on
        500 lastest transactions, earning you a total of{' '}
        {<span>{formatCurrency(allowBTC.amount.point, 0)}</span>} points.
      </p>
    ) : (
  ...
  const onClickShare = async () => {
    if (isNeedClaimBTCPoint) {
      shareTw({
        fee: allowBTC.amount.fee,
        point: allowBTC.amount.point,
