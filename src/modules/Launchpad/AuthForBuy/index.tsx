...
const AuthForBuy: React.FC<IAuthForBuy> = ({launchpadDetail}) => {
  const user = useAppSelector(userSelector);
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [hasStaked, setHasStaked] = useState(false);
  const { userContributeInfo } = useLaunchpadContext();
  ...
  const textAction = useMemo(() => {
    if(Number(userContributeInfo?.usdt_value) > 0) {
      return `Buy $${launchpadDetail?.token?.symbol}`;
    }
    ...
    <>
      ...
      >
        <BaseModal
          isShow={isOpen}
          onHide={onClose}
          title={isSigned ? textAction : textAction}
          headerClassName={s.modalHeader}
          className={cs(s.modalContent, isSigned ? s.deposit : s.notSignModal, { [s.banned]: isBanned })}
          // size={modalSize}
        >
          <DepositContent hasStaked={hasStaked} setBanned={setIsBanned} launchpadDetail={launchpadDetail}/>
