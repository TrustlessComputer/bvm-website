...
const ContributorInfo = ({ launchpadDetail, data, blockReward }: { launchpadDetail: ILaunchpadDetail, data?: ILeaderBoardPoint, blockReward?: any }) => {
  ...
  return (
    <Flex direction={'column'} w={'284px'} gap={3} className={s.container}>
      ...
      {isEnded && (
        <HorizontalItem className={s.rowData} color={"#000000"} label={'ALLOCATION'} value={
          <Flex flexDir="column" gap="2px">
            <Text>
              {formatCurrency(data?.bvm_balance, MIN_DECIMAL, MIN_DECIMAL)} ${launchpadDetail?.token?.symbol}
            </Text>
            <Text>
      ...
      {Boolean(isEnded && !!Number(data?.bvm_lucky_balance || 0)) && (
        <HorizontalItem className={s.rowData} color={"#000000"} label={'RED PACKAGE REWARD'} value={`${formatCurrency(data?.bvm_lucky_balance, 0, MIN_DECIMAL)} ${launchpadDetail?.token?.symbol}`} />
      )}
      {Boolean(isEnded && !!Number(blockReward?.total || 0)) && (
        <HorizontalItem className={s.rowData} color={"#000000"} label={'EARLY REWARD'} value={`${formatCurrency(blockReward?.total, 0, MIN_DECIMAL)} ${launchpadDetail?.token?.symbol}`} />
      )}
