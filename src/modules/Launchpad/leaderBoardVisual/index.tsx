...
const LeaderBoardVisual = (props: IProps) => {
  ...
  const TOTALs = 31;
  const { launchpadLeaderBoardVisual, setCurrentLaunchpadLeaderBoardVisual } = useLaunchpadContext();
  ...
  const fetchData = async (isNew?: boolean) => {
    try {
      ...
      if (isNew) {
        ...
        setCurrentLaunchpadLeaderBoardVisual(arr);
      } else {
        const arr = sortList([...response]);
        setCurrentLaunchpadLeaderBoardVisual(arr);
      }
  ...
  useEffect(() => {
    ...
    const missingLength = (mobileScreen ? (TOTALs - 1) : TOTALs) - launchpadLeaderBoardVisual.length;
    ...
    let sortList = [...launchpadLeaderBoardVisual].sort((a, b) => {
  ...
  }, [launchpadLeaderBoardVisual, mobileScreen]);
