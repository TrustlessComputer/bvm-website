...
const GetBVMSection = ({
  ...props
}: TGetBVMSectionProps): React.JSX.Element => {
  ...
  return (
    <div className={`${s.wrapper}`}>
      <div className={'container'}>
        <Flex
          className={`${s.wrapperSection}`}
          flex={1}
          flexDirection={props.rowReverse ? 'row-reverse' : 'row'}
          alignItems={props.rowReverse ? 'start' : 'center'}
        >
          ...
          <div className={`${s.wrapperContent}`}>
            ...
            {props.list ? (
              <Text fontSize={['20px']} color={'#000'}>
                <UnorderedList
                  listStylePosition={isDesktop ? 'outside' : 'inside'}
                >
                  <ListItem>
              ...
              <Text fontSize={['20px']} color={'#000'}>
                Ship better code in less time by bringing your tools, teammates
                and code changes together in Slack.
              </Text>
            )}
